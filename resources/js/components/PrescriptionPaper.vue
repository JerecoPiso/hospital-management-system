...
const caseNumber = computed(() => props.prescription?.patient_case?.case_number ?? props.prescription?.patientCase?.case_number ?? "—");
...
const drugStrength = (item: PrescriptionItem) => {
  ...
  if (!m) return "";
  return [m.dosage != null ? `${Number(m.dosage)} ${m.dosage_unit || ""}` : null, m.form].filter(Boolean).join(" ");
};

    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            <template #header>
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-linear-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-sm">
                        <FaBuilding class="text-white" size="16" />
                    </div>
                    <div>
                        <h2 class="text-base font-semibold text-slate-800">{{ isUpdate ? 'Edit Building' : 'New Building' }}</h2>
                        <p class="text-xs text-slate-400 mt-0.5">{{ isUpdate ? 'Update the building details' : 'Fill in the building details below' }}</p>
                    </div>
                </div>
            </template>
            <form @submit.prevent="isUpdate ? update() : create()" class="flex flex-col gap-5 pt-2">
                <div class="grid grid-cols-2 gap-x-4 gap-y-4">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <InputText v-model="info.code" placeholder="e.g. MAIN, ANNEX" required fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-sm font-medium text-slate-700">Name <span class="text-red-400">*</span></label>
                        <InputText v-model="info.name" placeholder="Building name" required fluid class="text-sm" />
                    </div>
                ...
                <div class="flex gap-2 pt-1">
                    ...
                    <Button type="submit" :label="isUpdate ? 'Update Building' : 'Save Building'" fluid class="bg-linear-to-r from-emerald-500 to-teal-600 border-0" />
                </div>
            </form>
        </Dialog>
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-linear-to-r from-slate-50 to-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-800">Buildings</h3>
        ...
        <DataTable :value="buildings" lazy paginator :rows="rows" :first="first" :totalRecords="total" :loading="loading" @page="onPage" :rowsPerPageOptions="[10, 15, 25, 50, 100]" responsiveLayout="scroll" tableStyle="min-width: 50rem"
            ...
            <template #empty>
                <div class="flex flex-col items-center justify-center py-16 text-slate-400">
                    <FaBuilding size="36" class="mb-3 opacity-30" />
                    <p class="text-sm font-medium">No buildings found</p>
                    <p class="text-xs mt-1">Click "Add Building" to create the first entry</p>
                </div>
            </template>
...
const defaultInfo = (): Building => ({ pid: '', code: '', name: '', description: '' });
const info = reactive<Building>(defaultInfo());
...
const edit = async (pid: string) => {
    try {
        await buildingStore.view(pid);
        Object.assign(info, buildingStore.building);
        isUpdate.value = true;
        modalOpen.value = true;
    } catch (err: any) {
        toast.error(err.response?.data?.message || 'Failed to retrieve building');
    }
};

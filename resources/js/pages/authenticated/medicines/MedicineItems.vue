<template>
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            <form @submit.prevent="isUpdate ? update() : create()" class="flex flex-col gap-5 pt-2">
                <div class="grid grid-cols-2 gap-x-4 gap-y-4">
                    <div class="col-span-2 flex flex-col gap-1.5">
                        ...
                        <InputText v-model="info.name" placeholder="Medicine name" required fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-sm font-medium text-slate-700">Generic Name</label>
                        <InputText v-model="info.generic_name" placeholder="e.g. Paracetamol" fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-sm font-medium text-slate-700">Brand Name</label>
                        <InputText v-model="info.brand_name" placeholder="e.g. Biogesic" fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-sm font-medium text-slate-700">Dosage</label>
                        <InputNumber v-model="info.dosage" :useGrouping="false" :minFractionDigits="2" placeholder="0.00" fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                ...
                <div class="flex gap-2 pt-1">
                    ...
                    <Button type="submit" :label="isUpdate ? 'Update Medicine' : 'Save Medicine'" fluid class="bg-linear-to-r from-emerald-500 to-teal-600 border-0" />
                </div>
            </form>
        </Dialog>
        ...
        <DataTable :value="medicines" lazy paginator :rows="rows" :first="first" :totalRecords="total" :loading="loading" @page="onPage" :rowsPerPageOptions="[10, 15, 25, 50, 100]" responsiveLayout="scroll" tableStyle="min-width: 60rem"
            ...
            <Column field="generic_name" header="Generic Name">
                <template #body="{ data }"><span class="text-slate-600 text-sm">{{ data.generic_name || '—' }}</span></template>
            </Column>
            <Column field="brand_name" header="Brand Name">
                <template #body="{ data }"><span class="text-slate-600 text-sm">{{ data.brand_name || '—' }}</span></template>
            </Column>
            ...
            <Column field="form" header="Form" class="w-32">
                <template #body="{ data }">
                    <span v-if="data.form" class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-teal-50 text-teal-700 border border-teal-100">{{ data.form }}</span>
                    <span v-else class="text-slate-300 text-sm italic">—</span>
                </template>
            </Column>
            <Column field="administration_route" header="Route" class="w-28">
                <template #body="{ data }">
                    <span v-if="data.administration_route" class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-slate-100 text-slate-600">{{ data.administration_route }}</span>
                    <span v-else class="text-slate-300 text-sm italic">—</span>
                </template>
            </Column>
...
const defaultInfo = (): Medicines => ({ pid: '', name: '', generic_name: '', brand_name: '', dosage: 0, dosage_unit: '', form: '', administration_route: '', price: 0 });
const info = reactive<Medicines>(defaultInfo());
...
const { search, rows, first, total, loading, onPage, onSearch, reload } = useApiTable(
    async (params) => {
        try {
            await medicineStore.read(params);
        } catch (err: any) {
...
const create = async () => {
    try {
        await medicineStore.create(info);
...
const edit = async (pid: string) => {
    try {
        ...
        Object.assign(info, medicineStore.medicine);
        isUpdate.value = true;
...
const update = async () => {
    try {
        await medicineStore.update(info);

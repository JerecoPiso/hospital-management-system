  ...
  <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
    ...
    <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-linear-to-r from-slate-50 to-white">
      ...
      <div class="flex items-center gap-2 w-full sm:w-auto">
        ...
        <button
          v-if="can('medicine-stocks', 'create')"
          type="button"
    ...
    >
      ...
      <Column header="Medicine">
        <template #body="{ data }"
          ><span class="text-slate-800 text-sm font-medium">{{ data.medicine?.name || "—" }}</span></template
        >
      </Column>
      ...
      <Column header="Expiration" class="w-32">
        <template #body="{ data }"
          ><span class="text-slate-600 text-sm">{{ data.expiration_date?.split("T")[0] || "—" }}</span></template
        >
      </Column>
      <Column header="Actions" class="w-24">
        <template #body="{ data }">
          <div class="flex items-center gap-1">
            <button
              v-if="can('medicine-stocks', 'update')"
              type="button"
              title="Edit stock"
              @click="edit(data.pid)"
            ...
            <button
              v-if="can('medicine-stocks', 'delete')"
              type="button"
              title="Delete stock"
...
const defaultInfo = (): MedicineStock => ({
  pid: "",
  medicine_pid: "",
  quantity: 0,
  purchase_price: null,
...
watch(expirationDate, (date) => {
  info.expiration_date = date ? date.toISOString().slice(0, 10) : null;
});

    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            <template #header>
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-linear-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-sm">
                        <Fa6TruckFast class="text-white" size="16" />
                    </div>
                    <div>
            ...
            <form @submit.prevent="create" class="flex flex-col gap-5 pt-2">
                <div class="grid grid-cols-2 gap-x-4 gap-y-4">
                    <div class="col-span-2 flex flex-col gap-1.5">
                        ...
                        <Select v-model="info.medicine_stock_pid" :options="medicineStocks" :optionLabel="stockOptionLabel" optionValue="pid" placeholder="Select stock batch" filter fluid class="text-sm" />
                    </div>
                    <div class="col-span-2 flex flex-col gap-1.5">
                        <label class="text-sm font-medium text-slate-700">Station <span class="text-red-400">*</span></label>
                        <Select v-model="info.station_pid" :options="stations" optionLabel="name" optionValue="pid" placeholder="Select station" filter fluid class="text-sm" />
                    </div>
                    <div class="col-span-2 flex flex-col gap-1.5">
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-linear-to-r from-slate-50 to-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-800">Medicine Distributions</h3>
                    <p class="text-xs text-slate-400">Track medicine stock distributed to stations</p>
                </div>
            </div>
            <div class="flex items-center gap-2 w-full sm:w-auto">
                ...
                <button v-if="can('medicine-distributions', 'create')" type="button" @click="modalOpen = true" class="flex items-center gap-2 px-4 py-2.5 rounded-lg transition-all duration-200 bg-linear-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-sm font-medium shadow-md hover:shadow-lg active:scale-95 shrink-0">
        ...
        <DataTable :value="medicineDistributions" lazy paginator :rows="rows" :first="first" :totalRecords="total" :loading="loading" @page="onPage" :rowsPerPageOptions="[10, 15, 25, 50, 100]" responsiveLayout="scroll" tableStyle="min-width: 60rem"
            ...
            <template #empty>
                <div class="flex flex-col items-center justify-center py-16 text-slate-400">
                    <Fa6TruckFast size="36" class="mb-3 opacity-30" />
                    <p class="text-sm font-medium">No distributions recorded</p>
                </div>
            </template>
            <Column header="Medicine">
                <template #body="{ data }"><span class="text-slate-800 text-sm font-medium">{{ data.medicineStock?.medicine?.name || '—' }}</span></template>
            </Column>
...
import { useMedicineDistributionStore } from '@/store/MedicineDistribution';
import { useMedicineStockStore } from '@/store/MedicineStock';
import { useStationStore } from '@/store/Station';
import { MedicineDistribution, MedicineStock } from '@/interface/Interfaces';
...
const medicineDistributionStore = useMedicineDistributionStore();
const medicineStockStore = useMedicineStockStore();
const stationStore = useStationStore();

const medicineDistributions = computed<MedicineDistribution[]>(() => medicineDistributionStore.medicineDistributions);
...
const defaultInfo = (): MedicineDistribution => ({ medicine_stock_pid: '', station_pid: '', quantity: 0 });
const info = reactive<MedicineDistribution>(defaultInfo());

watch(modalOpen, (open) => { if (!open) { Object.assign(info, defaultInfo()); } });

const formatDate = (value?: string) => value ? new Date(value).toLocaleString() : '—';
const stockOptionLabel = (data: MedicineStock) => `${data.medicine?.name || 'Unknown'}${data.batch_number ? ' — ' + data.batch_number : ''} (qty: ${data.quantity ?? 0})`;

const { search, rows, first, total, loading, onPage, onSearch, reload } = useApiTable(
    async (params) => {
        try {
            await medicineDistributionStore.read(params);
        } catch (err: any) {
            toast.error(err.response?.data?.message || 'Failed to retrieve distributions');
        }
    },
    () => medicineDistributionStore.meta,
);
...
const create = async () => {
    try {
        await medicineDistributionStore.create(info);
        toast.success('Stock distributed successfully');
        modalOpen.value = false;
        await reload();
        await medicineStockStore.read();
    } catch (err: any) {

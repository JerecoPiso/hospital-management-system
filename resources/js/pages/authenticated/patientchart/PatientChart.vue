            ...
            >
              <div v-if="profileOpen" class="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden z-50">
                ...
                <div class="py-2">
                  ...
                  <button @click="handleMenuClick('settings')" class="w-full px-6 py-3 text-left text-slate-700 hover:bg-slate-50 flex items-center gap-3 transition-colors">
                    <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                ...
                <!-- Logout -->
                <button @click="handleMenuClick('logout')" class="w-full px-6 py-3 text-left text-red-600 hover:bg-red-50 flex items-center gap-3 transition-colors font-medium">
          ...
          <template v-for="item in navItems" :key="item.name">
            ...
            <div v-if="item.children">
              ...
              >
                ...
                <svg v-if="sidebarExpanded" class="w-4 h-4 transition-transform shrink-0" :class="{ 'rotate-180': expandedMenu === item.name }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <router-link
              ...
              :title="!sidebarExpanded ? item.label : ''"
            >
              <component :is="item.icon" class="text-xl text-gray-600" :class="activeNav === item.name ? 'text-white' : 'text-gray-600'" />

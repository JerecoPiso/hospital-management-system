<template>
  ...
  <div v-else class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
    <Dialog v-model:visible="modalOpen" modal :style="{ width: '56vw' }" :breakpoints="{ '1199px': '85vw', '575px': '95vw' }" :pt="{ header: { class: 'border-b border-slate-100 pb-4' } }">
      <template #header>
          ...
          <div v-for="(item, index) in info.items" :key="index" class="rounded-lg border border-slate-200 p-4 flex flex-col gap-3 bg-slate-50/50">
            ...
            <div class="grid grid-cols-4 gap-3">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <Select v-model="item.frequency" :options="frequencies" optionLabel="label" optionValue="value" placeholder="Select frequency" class="w-full" />
              </div>
              <div class="flex flex-col gap-1.5">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <Select v-model="item.duration_unit" :options="durationUnits" placeholder="Select duration unit" class="w-full" />
              </div>
    ...
    >
      ...
      <Column header="Date" class="w-40">
        <template #body="{ data }"
          ><span class="text-slate-600 text-sm">{{ formatDateTime(data.prescription_date) }}</span></template
        >
      </Column>
...
const medicineOptionLabel = (data: Medicines) => `${data.name}${data.dosage ? " — " + Number(data.dosage) + " " + (data.dosage_unit || "") : ""}`;
...
const frequencies = computed(() => listStore.frequencies);
const durationUnits = computed(() => listStore.durationUnits);

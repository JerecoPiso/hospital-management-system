<template>
  ...
  <div v-else class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
    <Dialog v-model:visible="noteModal" modal :style="{ width: '48vw' }" :breakpoints="{ '1199px': '75vw', '575px': '95vw' }" :pt="{ header: { class: 'border-b border-slate-100 pb-4' } }">
      <template #header>
      ...
      <form @submit.prevent="isUpdate ? update() : create()" class="flex flex-col gap-5 pt-2">
        <div class="grid grid-cols-2 gap-4">
          <div class="flex flex-col gap-1.5">
            <label class="text-sm font-medium text-slate-700"> Focus <span class="text-red-400">*</span> </label>
            <Textarea v-model="nursesNotesInfo.focus" rows="3" autoResize fluid required placeholder="Problem or concern..." class="text-sm" />
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="text-sm font-medium text-slate-700"> Data <span class="text-red-400">*</span> </label>
            <Textarea v-model="nursesNotesInfo.data" rows="3" autoResize fluid required placeholder="Subjective / objective data..." class="text-sm" />
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="text-sm font-medium text-slate-700"> Action <span class="text-red-400">*</span> </label>
            <Textarea v-model="nursesNotesInfo.action" rows="3" autoResize fluid required placeholder="Nursing interventions taken..." class="text-sm" />
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="text-sm font-medium text-slate-700">Response</label>
            <Textarea v-model="nursesNotesInfo.response" rows="3" autoResize fluid placeholder="Patient's response..." class="text-sm" />
          </div>
        </div>
        <div class="flex gap-2 pt-1">
          <Button type="button" label="Cancel" severity="secondary" outlined fluid @click="noteModal = false" />
    ...
    >
      ...
      <Column header="Nurse" class="w-52">
        <template #body="{ data }">
          <div class="flex items-center gap-2.5">
            <div class="w-7 h-7 rounded-full bg-linear-to-br from-teal-400 to-emerald-500 flex items-center justify-center text-white text-xs font-semibold shrink-0">
              {{ data.nurse?.firstname?.charAt(0) ?? "?" }}
            </div>
            <span class="text-slate-700 text-sm font-medium leading-tight">
              {{ `${data.nurse?.firstname ?? ""} ${data.nurse?.middlename ? data.nurse.middlename + " " : ""}${data.nurse?.lastname ?? ""}`.trim() || "—" }}
            </span>
          </div>
        </template>
      </Column>
      ...
      <Column header="Actions" class="w-24">
        <template #body="{ data }">
          <div class="flex items-center gap-1">
            <button v-if="can('nurses-notes', 'update')" type="button" title="Edit note" @click="view(data.pid)" class="p-1.5 rounded-md text-teal-600 hover:bg-teal-50 hover:text-teal-700 transition-colors duration-150 cursor-pointer">
              <BiEdit size="18" />
            </button>
            <button v-if="can('nurses-notes', 'delete')" type="button" title="Delete note" @click="archive(data.pid)" class="p-1.5 rounded-md text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors duration-150 cursor-pointer">
              <BiTrash size="18" />
            </button>
          </div>
        </template>

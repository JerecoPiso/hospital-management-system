    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <Dialog v-model:visible="modalOpen" modal :style="{ width: '38vw' }" :breakpoints="{ '1199px': '75vw', '575px': '95vw' }" :pt="{ header: { class: 'border-b border-slate-100 pb-4' } }">
            <template #header>
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-base font-semibold text-slate-800">{{ isUpdate ? 'Edit Diet' : 'New Diet' }}</h2>
                        <p class="text-xs text-slate-400 mt-0.5">{{ isUpdate ? 'Update the diet details' : 'Fill in the diet details below' }}</p>
                    </div>
                </div>
            </template>
            <form @submit.prevent="isUpdate ? update() : create()" class="flex flex-col gap-5 pt-2">
                <div class="flex flex-col gap-1.5">
                    ...
                    <InputText v-model="info.name" placeholder="e.g. Diabetic Diet, Full Diet, NPO" required fluid class="text-sm" />
                </div>
                <div class="flex flex-col gap-1.5">
                ...
                <div class="flex gap-2 pt-1">
                    ...
                    <Button type="submit" :label="isUpdate ? 'Update Diet' : 'Save Diet'" fluid class="bg-linear-to-r from-emerald-500 to-teal-600 border-0" />
                </div>
            </form>
        </Dialog>
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-linear-to-r from-slate-50 to-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-800">Diets</h3>
                    <p class="text-xs text-slate-400">Manage the hospital diet catalog</p>
                </div>
            </div>
            <div class="flex items-center gap-2 w-full sm:w-auto">
                ...
                <button v-if="can('diets', 'create')" type="button" @click="modalOpen = true" class="flex items-center gap-2 px-4 py-2.5 rounded-lg transition-all duration-200 bg-linear-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-sm font-medium shadow-md hover:shadow-lg active:scale-95 shrink-0">
        ...
        <DataTable :value="diets" lazy paginator :rows="rows" :first="first" :totalRecords="total" :loading="loading" @page="onPage" :rowsPerPageOptions="[10, 15, 25, 50, 100]" responsiveLayout="scroll" tableStyle="min-width: 40rem"
            ...
            <template #empty>
                <div class="flex flex-col items-center justify-center py-16 text-slate-400">
                    <FaUtensils size="34" class="mb-3 opacity-30" />
                    <p class="text-sm font-medium">No diets found</p>
                </div>
            </template>
            ...
            <Column header="Actions" class="w-24">
                <template #body="{ data }">
                    <div class="flex items-center gap-1">
                        <button v-if="can('diets', 'update')" type="button" title="Edit" @click="edit(data.pid)" class="p-1.5 rounded-md text-teal-600 hover:bg-teal-50 hover:text-teal-700 transition-colors duration-150 cursor-pointer">
                            <BiEdit size="18" />
                        </button>
                        <button v-if="can('diets', 'delete')" type="button" title="Delete" @click="archive(data.pid)" class="p-1.5 rounded-md text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors duration-150 cursor-pointer">
                            <BiTrash size="18" />
                        </button>
                    </div>
                </template>
...
const defaultInfo = (): Diet => ({ pid: '', name: '', description: '' });
const info = reactive<Diet>(defaultInfo());
...
const create = async () => {
    try {
        await dietStore.create(info);
        toast.success('Diet created successfully');
        modalOpen.value = false;
        await reload();
    } catch (err: any) {
        toast.error(err.response?.data?.message || 'Failed to create diet');
    }
};

const edit = async (pid: string) => {
    try {
        await dietStore.view(pid);
        Object.assign(info, dietStore.diet);
        isUpdate.value = true;
        modalOpen.value = true;
    } catch (err: any) {
        toast.error(err.response?.data?.message || 'Failed to retrieve diet');
    }
};

const update = async () => {
    try {
        await dietStore.update(info);
        toast.success('Diet updated successfully');
        modalOpen.value = false;
        await reload();
    } catch (err: any) {
        toast.error(err.response?.data?.message || 'Failed to update diet');
    }
};
...
const archive = (pid: string) => {
    showConfirm({
        ...
        onAccept: async () => {
            try {
                await dietStore.archive(pid);
                toast.success('Diet deleted successfully');
                await reload();
            } catch (err: any) {
                toast.error(err.response?.data?.message || 'Failed to delete diet');
            }
        },
    });
};

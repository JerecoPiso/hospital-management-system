...
import { FiClock, FiCalendar, FiSearch, FiEye, FiPrinter } from "vue-icons-plus/fi";
...
const visitsTodayCount = computed(() => {
  ...
  return outpatients.value.filter((p) => isSameDay(p.patient_cases?.[0]?.admission_datetime, today)).length;
});
...
const visitsThisWeekCount = computed(() => {
  ...
  return outpatients.value.filter((p) => {
    const visitAt = p.patient_cases?.[0]?.admission_datetime;

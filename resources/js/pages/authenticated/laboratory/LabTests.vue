    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <Dialog v-model:visible="modalOpen" modal :style="{ width: '38vw' }" :breakpoints="{ '1199px': '75vw', '575px': '95vw' }" :pt="{ header: { class: 'border-b border-slate-100 pb-4' } }">
            <template #header>
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-base font-semibold text-slate-800">{{ isUpdate ? 'Edit Lab Test' : 'New Lab Test' }}</h2>
                        <p class="text-xs text-slate-400 mt-0.5">{{ isUpdate ? 'Update the lab test details' : 'Fill in the lab test details below' }}</p>
                    </div>
                </div>
            </template>
            <form @submit.prevent="isUpdate ? update() : create()" class="flex flex-col gap-5 pt-2">
                <div class="grid grid-cols-2 gap-x-4 gap-y-4">
                    <div class="col-span-2 flex flex-col gap-1.5">
                        ...
                        <Select v-model="info.category_pid" :options="categories" optionLabel="name" optionValue="pid" placeholder="Select category" filter fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-sm font-medium text-slate-700">Code <span class="text-red-400">*</span></label>
                        <InputText v-model="info.code" placeholder="e.g. CBC-01" required fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-sm font-medium text-slate-700">Name <span class="text-red-400">*</span></label>
                        <InputText v-model="info.name" placeholder="e.g. Complete Blood Count" required fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                ...
                <div class="flex gap-2 pt-1">
                    ...
                    <Button type="submit" :label="isUpdate ? 'Update Lab Test' : 'Save Lab Test'" fluid class="bg-linear-to-r from-emerald-500 to-teal-600 border-0" />
                </div>
            </form>
        </Dialog>
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-linear-to-r from-slate-50 to-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-800">Lab Tests</h3>
                    <p class="text-xs text-slate-400">Manage laboratory test catalog and pricing</p>
                </div>
            </div>
            <div class="flex items-center gap-2 w-full sm:w-auto">
                ...
                <button v-if="can('lab-tests', 'create')" type="button" @click="modalOpen = true" class="flex items-center gap-2 px-4 py-2.5 rounded-lg transition-all duration-200 bg-linear-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-sm font-medium shadow-md hover:shadow-lg active:scale-95 shrink-0">
        ...
        <DataTable :value="tests" lazy paginator :rows="rows" :first="first" :totalRecords="total" :loading="loading" @page="onPage" :rowsPerPageOptions="[10, 15, 25, 50, 100]" responsiveLayout="scroll" tableStyle="min-width: 55rem"
            ...
            <template #empty>
                <div class="flex flex-col items-center justify-center py-16 text-slate-400">
                    ...
                    <p class="text-sm font-medium">No lab tests found</p>
                    <p class="text-xs mt-1">Click "Add Lab Test" to create the first entry</p>
                </div>
            </template>
            <Column header="Category">
                <template #body="{ data }"><span class="text-slate-800 text-sm font-medium">{{ data.category?.name || '—' }}</span></template>
            </Column>
            ...
            <Column header="Actions" class="w-24">
                <template #body="{ data }">
                    <div class="flex items-center gap-1">
                        <button v-if="can('lab-tests', 'update')" type="button" title="Edit" @click="edit(data.pid)" class="p-1.5 rounded-md text-teal-600 hover:bg-teal-50 hover:text-teal-700 transition-colors duration-150 cursor-pointer">
                            <BiEdit size="18" />
                        </button>
                        <button v-if="can('lab-tests', 'delete')" type="button" title="Delete" @click="archive(data.pid)" class="p-1.5 rounded-md text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors duration-150 cursor-pointer">
                            <BiTrash size="18" />
                        </button>
                    </div>
                </template>
...
import { GiTestTubes } from 'vue-icons-plus/gi';
import { useLabTestStore } from '@/store/LabTest';
import { useLabTestCategoryStore } from '@/store/LabTestCategory';
...
const labTestCategoryStore = useLabTestCategoryStore();

const tests = computed<LabTest[]>(() => labTestStore.tests);
const categories = computed(() => labTestCategoryStore.categories);
const modalOpen = ref<boolean>(false);
const isUpdate = ref<boolean>(false);
const defaultInfo = (): LabTest => ({ pid: '', category_pid: '', code: '', name: '', price: 0, is_active: true });
const info = reactive<LabTest>(defaultInfo());
...
const { search, rows, first, total, loading, onPage, onSearch, reload } = useApiTable(
    async (params) => {
        try {
            await labTestStore.read(params);
        } catch (err: any) {
            toast.error(err.response?.data?.message || 'Failed to retrieve lab tests');
        }
    },
    () => labTestStore.meta,
);
...
onMounted(() => {
    labTestCategoryStore.read();
});

const create = async () => {
    try {
        await labTestStore.create(info);
...
const edit = async (pid: string) => {
    try {
        await labTestStore.view(pid);
        Object.assign(info, labTestStore.test, { category_pid: labTestStore.test.category?.pid || '' });
        isUpdate.value = true;
        modalOpen.value = true;
    } catch (err: any) {
        toast.error(err.response?.data?.message || 'Failed to retrieve lab test');
    }
};

const update = async () => {
    try {
        await labTestStore.update(info);
...
const archive = (pid: string) => {
    showConfirm({
        ...
        onAccept: async () => {
            try {
                await labTestStore.archive(pid);

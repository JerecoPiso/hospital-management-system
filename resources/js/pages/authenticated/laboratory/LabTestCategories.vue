            ...
            <template #header>
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-linear-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-sm">
                        <FaFlask class="text-white" size="15" />
                    </div>
                    <div>
                        <h2 class="text-base font-semibold text-slate-800">{{ isUpdate ? 'Edit Category' : 'New Category' }}</h2>
                        <p class="text-xs text-slate-400 mt-0.5">{{ isUpdate ? 'Update the lab test category details' : 'Fill in the category details below' }}</p>
                    </div>
                </div>
            </template>
            <form @submit.prevent="isUpdate ? update() : create()" class="flex flex-col gap-5 pt-2">
                <div class="flex flex-col gap-4">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <InputText v-model="info.name" placeholder="e.g. Hematology, Microbiology" required fluid class="text-sm" />
                    </div>
                    <div class="flex flex-col gap-1.5">
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-linear-to-r from-slate-50 to-white">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-linear-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-md">
                    <FaFlask class="text-white" size="17" />
                </div>
                <div>
                    <h3 class="text-base font-bold text-slate-800">Lab Test Categories</h3>
                    <p class="text-xs text-slate-400">Group laboratory tests by discipline</p>
                </div>
            </div>
        ...
        <DataTable :value="categories" lazy paginator :rows="rows" :first="first" :totalRecords="total" :loading="loading" @page="onPage" :rowsPerPageOptions="[10, 15, 25, 50, 100]" responsiveLayout="scroll" tableStyle="min-width: 40rem"
            ...
            <template #empty>
                <div class="flex flex-col items-center justify-center py-16 text-slate-400">
                    <FaFlask size="36" class="mb-3 opacity-30" />
                    <p class="text-sm font-medium">No lab test categories found</p>
            ...
            <Column header="Actions" class="w-24">
                <template #body="{ data }">
                    <div class="flex items-center gap-1">
                        <button v-if="can('lab-test-categories', 'update')" type="button" title="Edit" @click="edit(data.pid)" class="p-1.5 rounded-md text-teal-600 hover:bg-teal-50 hover:text-teal-700 transition-colors duration-150 cursor-pointer">
                            <BiEdit size="18" />
                        </button>
                        <button v-if="can('lab-test-categories', 'delete')" type="button" title="Delete" @click="archive(data.pid)" class="p-1.5 rounded-md text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors duration-150 cursor-pointer">
                            <BiTrash size="18" />
                        </button>
                    </div>
                </template>
...
import { FaFlask } from 'vue-icons-plus/fa';
...
const categories = computed<LabTestCategory[]>(() => labTestCategoryStore.categories);
const modalOpen = ref<boolean>(false);
const isUpdate = ref<boolean>(false);
const defaultInfo = (): LabTestCategory => ({ pid: '', name: '', description: '' });
const info = reactive<LabTestCategory>(defaultInfo());
...
const { search, rows, first, total, loading, onPage, onSearch, reload } = useApiTable(
    async (params) => {
        try {
            await labTestCategoryStore.read(params);
        } catch (err: any) {
            toast.error(err.response?.data?.message || 'Failed to retrieve lab test categories');
        }
    },
    () => labTestCategoryStore.meta,
);

const create = async () => {
    try {
        await labTestCategoryStore.create(info);
...
const edit = async (pid: string) => {
    try {
        await labTestCategoryStore.view(pid);
        Object.assign(info, labTestCategoryStore.category);
        isUpdate.value = true;
...
const update = async () => {
    try {
        await labTestCategoryStore.update(info);
...
const archive = (pid: string) => {
    showConfirm({
        ...
        onAccept: async () => {
            try {
                await labTestCategoryStore.archive(pid);
